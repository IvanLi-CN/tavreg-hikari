...
function readPasswordAfterEmail(value: string): string {
  return trimIgnoredPasswordSuffix(value.trimEnd().replace(LEADING_BOUNDARY_PATTERN, "").trim());
}

function readPasswordBeforeEmail(value: string): string {
  return trimIgnoredPasswordSuffix(value.trimStart().replace(TRAILING_BOUNDARY_PATTERN, "").trim());
}

function trimIgnoredPasswordSuffix(password: string): string {
  const separatorIndex = password.indexOf("----");
  if (separatorIndex < 0) return password;
  return password.slice(0, separatorIndex).trim();
}
