...
- Only run `bun run broker:sessions:reconcile -- --apply --db <sqlite>` after reviewing the dry-run list. It must close only sessions that are not referenced by active attempts, and it should re-read DB references plus active browser bootstrap guards and unbound running attempt launch guards immediately before each close to avoid scheduler launch/probe and mailbox/browser bootstrap windows.
- Reconciliation tooling must open the existing SQLite file without startup stale-state recovery. A dry-run must never mark running jobs failed before computing Broker references.
...
- Broker-open and domain-probe failures after an attempt is intentionally committed should complete the attempt as failed with the original `proxy_broker_*` or `proxy_domain_unreachable` code. Do not leave the attempt running as `spawned` with null proxy fields.
- For Microsoft-account-consuming Tavily batch launches, a `proxy_broker_*` failure before the worker child is spawned is infrastructure setup failure, not account business failure. Roll back the pending attempt, release the account lease, fail the job with a clear launch-setup error, and keep the account ready for a later run.
...
- If a worker writes a terminal artifact (`result.json` with an API key or `error.json`) but the detached process remains alive, the scheduler should wait only a short settle window, then complete the attempt from the artifact and signal the child for cleanup. Otherwise successful Tavily attempts can hold Broker sessions and keep the job running indefinitely while the API key is already on disk.
- Stale running or completing attempts must be reaped to terminal state and release their Broker session; paused jobs may retain live attempts until the job resumes or is stopped. For flows with sparse stdout, use a heartbeat or stage marker rather than stdout silence alone, and seed the stale timer from worker registration/progress rather than the earlier attempt creation timestamp.
