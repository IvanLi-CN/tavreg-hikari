...
- Sync lightweight worker stage markers, such as `stage.json.stage`, during ordinary running loop ticks before evaluating terminal cleanup so the attempt ledger stays useful while the worker is still alive.
...
- For task-scoped workers with an internal attempt timeout, write the signup-task ledger as a terminal failure before cleanup and then exit the worker process. Browser cleanup is best effort and must have a hard grace timer, because an already-timed-out worker can otherwise keep the scheduler row `running` until an external reaper or operator kills it.
...
- Do not rely on an async timeout callback merely closing browser resources. If the timed-out operation does not observe that close promptly, the child process can remain alive while the ledger already says `task_attempt_timeout`.
