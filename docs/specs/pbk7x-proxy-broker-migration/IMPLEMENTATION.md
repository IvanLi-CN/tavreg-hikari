...
- 代理页 API 从本地 Mihomo sync/check 改为读取 Broker catalog 与 active sessions，并把 catalog 快照写入现有 proxy diagnostics 表供历史查询。
...
- 代理页 Storybook 覆盖 catalog loaded、empty、auth error 与设置保存交互。
