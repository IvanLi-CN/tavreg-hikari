...
- `bun run broker:sessions:reconcile -- --db <sqlite> --dry-run|--apply` 提供受控清理入口；默认 dry-run，只有显式 `--apply` 才关闭 DB 未引用的 Broker sessions；apply 关闭每个 session 前会重新读取 DB 引用，避免 attempt launch 窗口误关新 session。
...
- Reconciler 单元测试覆盖 referenced session 保留、orphan session dry-run 不关闭、apply 仅关闭 orphan，以及 DB active reference 过滤终态 job。
