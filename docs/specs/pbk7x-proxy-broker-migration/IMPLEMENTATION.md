...
- `bun run broker:sessions:reconcile -- --db <sqlite> --dry-run|--apply` 提供受控清理入口；默认 dry-run，只有显式 `--apply` 才关闭 DB 未引用的 Broker sessions。
...
- 调度器回归测试覆盖 Tavily completing stale attempt、Grok stale attempt 与 ChatGPT stale attempt 的两阶段 force-stop / releaseResources 收敛。
