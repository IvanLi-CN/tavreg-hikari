...
- 对不通过 `signup_tasks` 的 ChatGPT/Grok worker，scheduler 必须在 running loop 中读取 worker `stage.json.stage` 并回写 `job_attempts.stage`；代理未绑定、代理已绑定与 worker 已 spawn 必须分别用 `allocating_proxy`、`proxy_bound`、`spawned` 表示。
...
- 上述 login 降级只允许发生在“仅残留无交互能力的 challenge 外壳”上；若页面仍存在可点击 checkbox、Turnstile runtime 或其他真实 challenge 信号，则必须继续等待 challenge 完成，不能把它当作可安全绕过的被动壳。
...
- Given ChatGPT/Grok attempt 正在启动代理，When Broker session 尚未成功绑定，Then attempt stage 为 `allocating_proxy` 且代理字段为空；Broker 失败时 attempt 进入 failed，而不是显示为未绑定代理的 `spawned`。
...
- Given Tavily login 页仍显示真实的 challenge checkbox、Turnstile runtime 或其他活跃 challenge 信号，When 被动 challenge 超时，Then provider 流程仍会继续等待 challenge，而不会把该页面误判为可直接重提 provider。
