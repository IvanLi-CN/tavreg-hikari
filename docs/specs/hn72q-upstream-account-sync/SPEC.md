...
- 生产实例已经长期维护 Microsoft 账号、proof mailbox、Tavily API key、ChatGPT credentials、Grok SSO key 与服务接入快照。
...
- 同步账号密码、分组、proof mailbox、账号状态摘要、Tavily key、ChatGPT credentials、Grok SSO key 与 Tavily service access snapshot。
- 保留本地运行态：browser profile path、lease、job history 与本地 session 状态不由线上快照覆盖。
- 仅在本地 Tavily / ChatGPT / Grok 成功时，把成功结果回写线上；失败、禁用、分组编辑与密码编辑不回写。
...
- 不同步 Grok / ChatGPT 失败记录、job history 或浏览器工件。
...
- `upstreamTavregWriteback`
  - 默认：`off`
  - `success_only` 时，本地 Tavily / ChatGPT / Grok 成功结果允许回写线上。
...
- `POST /api/upstream-sync/accounts`
  - 范围：`internal`
  - 行为：从 settings 保存的 upstream base URL 分页拉取 `/api/integration/v1/microsoft-accounts` 与 `/api/integration/v1/keys?site=...`，再逐详情 upsert 本地库。
  - 响应：返回 `created / updated / total / linkedApiKeys / syncedKeys / upstreamOrigin / startedAt / completedAt`。
...
- `GET /api/integration/v1/keys?site=tavily|chatgpt|grok`
  - 列表返回指定站点成功 key 的分页摘要，不返回 secret。
- `GET /api/integration/v1/keys/:site/:id`
  - Tavily detail 返回 API key、Microsoft account 关联摘要、service access snapshot。
  - ChatGPT detail 返回 email、accountId、accessToken、refreshToken、idToken、expiresAt、credentialJson。
  - Grok detail 返回 email、password、sso、ssoRw、cfClearance、checkoutUrl、birthDate 与提取时间/IP。
...
- `POST /api/integration/v1/keys/:site/success`
  - 接受 Tavily / ChatGPT / Grok 成功结果。
  - ChatGPT / Grok 请求必须包含 `sourceOrigin` 与 `sourceKeyId`，服务端以该映射幂等 upsert。
...
- 同步导入的 ChatGPT / Grok key 写入现有 `chatgpt_credentials` / `grok_api_keys`；本地通过 hidden synthetic completed job/attempt 满足既有外键，不导入线上 job history。
- 回写失败必须非致命：本地成功结果仍保留在本地，只在 UI/toast/log 里暴露失败原因。

## 验收标准

- Given 本地启用了 upstream sync 且配置了有效 API key，When 在 `/accounts` 点击“从线上同步”，Then 本地账号池出现线上账号、分组、proof mailbox、Tavily key 与服务摘要，Keys 页出现线上 ChatGPT/Grok 成功 key。
- Given 线上账号有 ready session，When 同步到本地，Then 本地 session 不会被标为 ready，仍需本地 bootstrap/profile。
- Given 本地 Tavily / ChatGPT / Grok 单账号或批量 attempt 成功，When settings 中 `upstreamTavregSyncEnabled=true` 且 `upstreamTavregWriteback=success_only`，Then 成功结果回写线上并保持本地成功非回滚语义。
...
- Unit/integration tests 覆盖：上游同步 upsert、integration keys read/detail、Tavily key 幂等记录、ChatGPT/Grok key 幂等记录、写回失败非致命。
