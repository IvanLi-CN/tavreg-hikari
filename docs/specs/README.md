...
| vyg62 | Release 失败 Oidrune 告警接入 | 已完成 | `vyg62-release-failure-telegram-alerts/SPEC.md` | 2026-09-01 | 固定 Oidrune notify.yml SHA、调用方完整摘要、OIDC 权限与 workflow contract test |
| k9tfr | 显式指纹浏览器契约 + GitHub 质量门禁收口 | 已完成 | `k9tfr-explicit-fingerprint-browser-quality-gates/SPEC.md` | 2026-04-25 | 显式 `CHROME_EXECUTABLE_PATH`、跨平台安装脚本、Docker 内置指纹浏览器、CI/Release/branch rules 闭环，并补齐 GHCR 公开 tag 的匿名可读 index 平台元数据契约 |
...
| svjx5 | 微软账号自动提取与本地历史接入 | 已完成 | `svjx5-microsoft-account-auto-extractor/SPEC.md` | 2026-04-19 | 四源适配、自动补号、本地历史、提号取消与视觉证据已完成收口 |
