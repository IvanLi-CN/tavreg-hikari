...
- 对紧凑卡片布局中的可变高度记录（如长 SSO / 长 KEY / 长邮箱）必须启用真实高度测量，避免虚拟列表仅靠固定估算时出现卡片互相重叠或分页信息被覆盖。
...
- source_type: `storybook_canvas`
  ...
  evidence_note: 验证 Grok keys 在 820px 视口已进入 `md`，恢复表格布局，同时隐藏 `出口 IP` 以优先保留关键字段宽度。
...
- source_type: `storybook_canvas`
  ...
  evidence_note: 验证 Grok keys 在更宽桌面视口下恢复 `出口 IP` 列，并把更多剩余空间分配给 SSO 和时间列，减少右侧空间浪费感。
