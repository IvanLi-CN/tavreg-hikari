...
- 主流程页：使用一颗上下文主按钮在 `启动 / 暂停 / 恢复` 之间切换，额外提供 `停止`（优雅停）与 `强行停止`（二次确认）控制，支持动态调整 `parallel / need / maxAttempts`，并在缺号时按站点开关自动提取微软账号；当前支持 `账号鸭 / 闪邮箱 / 闪客云 / Hotmail666` 四个渠道，按单源 `500ms/次` 轮转、最多 `4` 个并发请求。
