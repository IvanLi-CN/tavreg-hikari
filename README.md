...
- 主流程页：启动任务、暂停、恢复、动态调整 `parallel / need / maxAttempts`，并支持在缺号时按站点开关自动提取微软账号，配置提取数量、等待时长并查看实时补号状态。
- 微软账号页：支持 `email,password`、`email:password`、`email|password` 或 `email password` 的批量导入，按邮箱去重，查看是否已有 API key、最近状态、导入时间与跳过原因，并提供账号鸭 / 闪邮箱提取器 KEY 设置与本地提取历史查询。
...
- 如果 worktree 里还没有 `node_modules`，bootstrap 还会自动执行依赖安装；存在 `bun.lock` 时固定走 `bun install --frozen-lockfile`，否则回退到 `bun install`。手工 `WORKTREE_SYNC_FORCE=1` 重跑时若 `node_modules` 已存在，只会保留现状，不会强制重装；安装失败也只记日志，不会让 checkout 直接失败。
