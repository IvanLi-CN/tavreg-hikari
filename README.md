...
- 账号 session bootstrap 使用有界并发池，默认同时处理 3 个账号；可通过 `.env.local` 的 `MICROSOFT_ACCOUNT_BOOTSTRAP_LOGIN_MODE`、`MICROSOFT_ACCOUNT_BOOTSTRAP_CONCURRENCY`、`MICROSOFT_ACCOUNT_BOOTSTRAP_WORKER_TIMEOUT_MS`、`MICROSOFT_ACCOUNT_BOOTSTRAP_KILL_GRACE_MS`，或 Microsoft Graph 设置页调整登录方案、并发、worker 超时和强杀宽限。默认登录方案为 `microsoft_graph`，只要求完成 Graph callback/token 写入并保留 profile 登录态；`tavily_home` 作为兼容方案继续要求回到 Tavily Home。
