...
afterEach(async () => {
  ...
  if (originalProxyBrokerApiKey === undefined) {
...
test("chatgpt running reaper uses worker registration progress instead of attempt creation time", async () => {
  const { appDb, tempDir } = await createTempDb();
  const scheduler = new ChatGptJobScheduler(appDb, process.cwd(), () => createSchedulerSettings(), () => undefined);
  (scheduler as any).ensureLoop = () => undefined;
  const job = appDb.createJob({
  ...
  const attempt = appDb.createAttempt(job.id, {
    accountEmail: "fresh-worker-chatgpt@example.test",
    outputDir: path.join(tempDir, "fresh-worker-chatgpt-attempt"),
  });
  (appDb as any).db
  ...
  (scheduler as any).activeAttempts.set(attempt.id, {
    ...
    brokerSession: { session: { session_id: "sess-fresh-chatgpt" } },
    stopRequested: null,
  ...
  expect(appDb.getAttempt(attempt.id)?.status).toBe("running");
  expect((scheduler as any).activeAttempts.size).toBe(1);

  (scheduler as any).activeAttempts.clear();
  await scheduler.shutdown();
  appDb.close();
});
